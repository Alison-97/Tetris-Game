...
let pauseBtnRound = document.getElementById('pause-btn')
...
function start(){
    if(idleMode && !instructionMode){
        ...
        pauseBtnRound.style.visibility = "visible"
    }
}
...
function pauseAndPlay(event){
    // event.stopPropagation() //so that it wont trigger the rotate() on parent div (mobile)
...
function pauseScreen(){
    ...
    pauseBtnRound.style.visibility = "hidden"
}
...
//for mobile device
body.addEventListener("click", event => {}); // can prevent double-tap to zoom in playMode (somehow)
...
document.addEventListener('touchmove', e => {
    let swipeDistanceY = e.changedTouches[0].pageY - touchY
    let swipeDistanceX = e.changedTouches[0].pageX - touchX

    // from touchstart
    if (swipeDistanceX < -touchThreshold && keys.indexOf('swipeLeft') == -1) {
        keys.push('swipeLeft')
        moveLeft()
        currentTouchX = e.changedTouches[0].pageX
    }
     if (swipeDistanceX > touchThreshold && keys.indexOf('swipeRight') == -1) {
        keys.push('swipeRight')
        moveRight()
        currentTouchX = e.changedTouches[0].pageX
    }
     if (swipeDistanceY > touchThreshold && keys.indexOf('swipeDown') == -1) {
        keys.push('swipeDown')
        moveDown()
        currentTouchY = e.changedTouches[0].pageY
        // console.log('firstDown')
    }

    //from currenttouch (touchmove)
    let moveDistanceX = e.changedTouches[0].pageX - currentTouchX
    let moveDistanceY = e.changedTouches[0].pageY - currentTouchY
    if(moveDistanceX < -20 ) {
        moveLeft()
        currentTouchX = e.changedTouches[0].pageX
    }
...
document.addEventListener('touchend', e => {
    ...
    if (swipeDistanceY > touchThreshold && keys.indexOf('swipeLeft') == -1 && keys.indexOf('swipeRight') == -1) {
        hardDrop()
    }

    if (swipeDistanceY < -touchThreshold && keys.indexOf('swipeUp') == -1) {
        keys.push('swipeUp')
        if (playMode) {
            if(!holding){
                hold()
    ...
    if (swipeDistanceY==0 && swipeDistanceX==0){
        rotate()
    }
    keys = []
})
